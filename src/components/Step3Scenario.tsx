...
export default function Step3Scenario({ currentLand, currentRelaxation, onScenarioChange, activeStep = 3, savedScenario }: Step3ScenarioProps) {
  ...
  // Automatically sync manual undergroundFloors state to the calculated recommended value when recommendation changes
  useEffect(() => {
    if (result?.calculatedUndergroundFloors !== undefined && result.calculatedUndergroundFloors >= 0) {
      if (isInitialSyncRef.current) {
        isInitialSyncRef.current = false;
        if (inputs?.undergroundFloors === undefined) {
          setUndergroundFloors(result.calculatedUndergroundFloors);
        }
      } else {
        setUndergroundFloors(result.calculatedUndergroundFloors);
      }
    }
  }, [result?.calculatedUndergroundFloors, inputs?.undergroundFloors]);
                      ...
                      <div className="space-y-3.5 text-xs">
                        ...
                        <div className="p-2.5 bg-emerald-50/50 border border-emerald-100 rounded-lg space-y-1">
                          ...
                          <div className="flex justify-between items-center text-[10px] pt-1 border-t border-emerald-100/50">
                            ...
                            <span className="font-extrabold text-amber-700 text-xs text-right">
                              {floorCalculationMode === 'auto' ? (
                                <span>지하 {result.undergroundFloors} 층 <span className="text-[9px] font-normal text-amber-600">(자동 산정)</span></span>
                              ) : (
                                <span className="flex flex-col items-end">
                                  <span>지하 {result.undergroundFloors} 층 <span className="text-[9px] font-normal text-gray-500">(설정)</span></span>
                                  <span className="text-[9px] font-semibold text-amber-600 mt-0.5">※ 산정 공식 권장: 지하 {result.calculatedUndergroundFloors} 층</span>
                                </span>
                              )}
                            </span>
                          </div>
            ...
            {activeStep === 3 && (
              <div className="p-5 bg-white rounded-2xl border border-gray-150 shadow-sm space-y-5 animate-fadeIn">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-100 pb-3">
                  ...
                  {/* Tab Selectors (Smooth scroll links) */}
                  <div className="flex flex-wrap gap-1 bg-gray-50 p-1 rounded-xl border border-gray-150">
                    <span className="text-[9.5px] text-gray-400 font-bold px-2 py-1 flex items-center">📂 보고서 바로가기:</span>
                    <button
                      type="button"
                      onClick={() => document.getElementById('section-general')?.scrollIntoView({ behavior: 'smooth' })}
                      className="px-2.5 py-1 text-[10px] font-bold rounded-lg transition-all cursor-pointer text-gray-550 hover:bg-white hover:text-gray-900 border border-transparent hover:border-gray-100 hover:shadow-xs"
                    >
                      종합 건축개요
                    </button>
                    <button
                      type="button"
                      onClick={() => document.getElementById('section-area')?.scrollIntoView({ behavior: 'smooth' })}
                      className="px-2.5 py-1 text-[10px] font-bold rounded-lg transition-all cursor-pointer text-gray-550 hover:bg-white hover:text-gray-900 border border-transparent hover:border-gray-100 hover:shadow-xs"
                    >
                      용도별 면적조서
                    </button>
                ...
                <div id="section-general" className="bg-white p-5 rounded-2xl border border-gray-150 shadow-xs space-y-4 animate-fadeIn">
                  <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                    <h4 className="text-xs font-bold text-gray-800 flex items-center gap-1.5">
                      <span className="w-1.5 h-3 bg-[#5F7161] rounded-xs"></span>
                      📋 종합건축개요 (General Specs)
                    </h4>
                    <span className="text-[10px] text-gray-400 font-mono">SECTION 01 / 02</span>
                  </div>
                 ...
                 {/* Section 2: 용도별 면적조서 */}
                <div id="section-area" className="bg-white p-5 rounded-2xl border border-gray-150 shadow-xs space-y-4 animate-fadeIn">
                  <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                    <h4 className="text-xs font-bold text-gray-800 flex items-center gap-1.5">
                      <span className="w-1.5 h-3 bg-[#5F7161] rounded-xs"></span>
                      📊 용도별 면적조서 (Area Specs)
                    </h4>
                    <span className="text-[10px] text-gray-400 font-mono">SECTION 02 / 02</span>
                  </div>
                  <div className="space-y-4 animate-fadeIn">
                    <div className="overflow-x-auto border border-gray-150 rounded-xl">
