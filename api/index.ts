...
3. 친근하면서도 고도로 격식 있고 객관적인 전문가 톤앤매너로 서술하세요.

[PDF 변환 및 레이아웃 출력 최적화 지시사항 (Markdown Layout Rules)]
- 분석 내용을 PDF로 매끄럽게 변환하기 위해 다음 마크다운 레이아웃 규칙을 엄격히 준수하여 텍스트를 구조화할 것:
  * 제목 (Heading): # (대제목)과 ## (소제목)을 사용하여 법적/사업적 위계질서를 명확히 할 것.
  * 강조 (Emphasis): 핵심 단어나 중요한 법적 수치, 가이드라인은 반드시 **볼드체**로 표기할 것.
  * 구조화 (Layout): 데이터 비교(예: 용도지역 간 건폐율/용적률 비교, 주차기준 대조 등)가 필요한 경우 반드시 표(Table) 형식을 사용할 것.
  * 글머리 기호 (Bullet points): 나열되는 자문 요약 및 리스크 체크리스트 정보는 글머리 기호를 적극 활용할 것.
  * 핵심 요약 (Blockquote): 자문의 핵심 요약 한 줄 또는 중요 권장 조치는 > 를 사용하여 박스 형태로 시각화할 것.
  * 스타일: 전문적이고 간결한 보고서 문체를 사용할 것. 불필요하고 장황한 서술형 수식어구는 과감히 생략하고 가독성을 극대화할 것.
  * 출처 표기: 언급되는 구체적인 조례나 법령의 근거 자료는 반드시 하단이나 본문 끝에 [출처: OOO 법률/조례 제O조] 형식으로 명시할 것.
